...
评审重点是：**三层边界、主链路、授权确认闭环、验证责任和交付节奏。**
...
原来的风险是：Agent拿到用户Cookie后，天然拥有用户在传统系统里的完整权限。方案核心是把Cookie收回到Agent Gateway：
...
和`整体方案流程图.md`里的初版流程相比，推荐口径是：**Copilot不显式拿Token再交给Agent，而是调Agent Gateway上的Agent入口，由网关生成Token并注入到转发请求里。** 这样更符合“Agent少改造、前后端解耦、开源Agent独立演进”的设计目标。

---

## 5. Agent Gateway职责与能力
...
1. **问题背景**：Cookie透传、Agent权限过大、工具调用不可控、A2A权限不清。
2. **核心理念**：Cookie收回网关，Agent只拿任务Token，策略中心动态决策，MCP执行鉴权。
3. **主链路**：Copilot调Agent Gateway入口，网关注入Token，Agent调MCP，MCP问策略中心并调传统API。
4. **Agent Gateway职责**：注册、入口代理、Token、Session、Cookie、A2A、确认通道。
5. **策略中心职责**：委托策略、工具标签、权限验证、数据范围、确认规则、委托链。
6. **MCP Gateway职责**：工具列表/调用、Token校验、鉴权执行、数据注入、Cookie还原、审计。
7. **接入模式**：直连Agent、BFF在前、开源Agent轻量适配。
8. **验证矩阵**：按场景验收，不只按模块自测。
9. **责任分工**：每个模块主责、配合、验收口径。
10. **交付计划**：接口冻结、P0开发、人在环/A2A、联调验收。
...
| 决策项 | 结论 | 负责人 | 截止时间 |
| --- | --- | --- | --- |
| Agent Gateway是否为统一入口 | 待定 | 项目负责人 | 本次评审 |
| P0是否包含A2A | 待定 | 项目负责人 | 本次评审 |
| 业务BFF默认接入模式 | 待定 | 业务后端负责人 | 本次评审 |
| 人在环确认入口归属 | 待定 | Copilot负责人 | 本次评审 |
| 策略配置页归属 | 待定 | 策略中心/Copilot负责人 | 本次评审 |
| 工具标签维护归属 | 待定 | MCP Gateway负责人 | 本次评审 |
| 接口冻结时间 | 待定 | 各模块负责人 | D+2 |
| 联调环境时间 | 待定 | QA/联调负责人 | D+7 |
| 验收演示时间 | 待定 | 项目负责人 | D+20 |
