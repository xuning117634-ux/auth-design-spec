...
**Agent Gateway返回Cookie给MCP Gateway：**
...
- `任务级Token机制`：Token与Task绑定，任务开始生成，任务结束失效
- `工具标签策略配置`：工具分类标签化，委托策略基于标签组合
- `环境维度强制注入`：Token包含环境维度，MCP Gateway自动注入
- `A2A委托链传递`：完整委托链传递，支持多级A2A调用
- `Cookie还原机制`：Session保存Cookie，MCP Gateway还原调用传统API
- `委托生命周期管理`：长期、临时、会话级委托，系统自动管理
- `实时确认机制`：高危工具强制用户实时确认
- `分层架构设计`：Agent Gateway、MCP Gateway、策略中心三层架构
...
- `用户在Copilot输入`："查询当前组织的数据源"
Copilot创建Task（taskId=task-001）
Copilot发送请求到Agent Gateway（包含Cookie、agentId）
- Agent Gateway解析Cookie，提取用户身份
- Agent Gateway生成Token（tokenId=token-001）
- Agent Gateway返回Token给Agent
...
- Copilot发送请求到Agent Gateway（包含过期Cookie）
- Agent Gateway解析Cookie
- Agent Gateway验证Cookie（调用IDaaS）
IDaaS返回Cookie无效
...
- Copilot发送请求到Agent Gateway（包含agentId=agent-999）
- Agent Gateway查询Agent注册表
- Agent未找到
...
- Agent调用datasource_list工具（携带Token）
- MCP Gateway验证Token有效性
- MCP Gateway检查工具分类（普通工具）
- MCP Gateway调用Agent Gateway换取Cookie
- MCP Gateway使用Cookie调用b2b-admin-service API
- API返回数据源列表
...
- Agent调用datasource_create工具（携带Token）
- MCP Gateway验证Token有效性
- MCP Gateway检查工具分类（敏感工具）
- MCP Gateway查询策略中心（委托策略）
- 策略中心返回委托权限范围
- MCP Gateway检查权限匹配（allowedTags包含工具标签）
- MCP Gateway调用Agent Gateway换取Cookie
- MCP Gateway调用API创建数据源
...
- MCP Gateway检查工具分类（敏感工具）
- MCP Gateway查询策略中心（委托策略）
- MCP Gateway检查权限匹配（READ_ONLY vs WRITE）
- 权限不匹配
...
- Agent调用datasource_delete工具（携带Token）
- MCP Gateway验证Token有效性
- MCP Gateway检查工具分类（高危工具）
- MCP Gateway返回403错误：USER_CONFIRMATION_REQUIRED
- Agent提示用户确认
- 用户点击确认链接
- 权限中心创建临时委托
...
- MCP Gateway调用Agent Gateway：POST /cookie/exchange
- Agent Gateway验证Token有效性
- Agent Gateway验证sessionId一致性
- Agent Gateway查询Session表
- Agent Gateway解密Cookie
- Agent Gateway返回Cookie
...
- MCP Gateway查询委托策略
- MCP Gateway获取数据范围
- MCP Gateway注入数据范围到工具参数
- MCP Gateway调用API（参数：{keyword="ISALES", appName="C00001-O0023", datasourceIds=[184,305]}）
...
- Agent调用工具（参数：{keyword="ISALES", appName="C00001-O9999"}）
- MCP Gateway解析Token的environmentContext
- MCP Gateway强制注入appName=C00001-O0023（覆盖Agent传入的错误值）
- MCP Gateway调用API
...
- Agent A调用Agent Gateway：POST /agent/call（targetAgentId=agent-002）
- Agent Gateway解析Agent A的Token
- Agent Gateway检查A2A权限
- Agent Gateway构建委托链：user→agent-001→agent-002
- Agent Gateway生成Agent B的Token
- Agent Gateway调用Agent B
- Agent B调用MCP工具（携带Agent B的Token）
- MCP Gateway解析委托链
- MCP Gateway鉴权通过
...
- Agent A调用Agent Gateway：POST /agent/call
- Agent Gateway解析Agent A的Token
- Agent Gateway检查A2A权限
- A2A权限不足
...
- Agent A调用Agent B
- Agent Gateway生成Agent B的Token（委托链：user→agent-001→agent-002）
- Agent B调用Agent C
- Agent Gateway生成Agent C的Token（委托链：user→agent-001→agent-002→agent-003）
- Agent C调用MCP工具
- MCP Gateway解析三级委托链
...
- 用户选择Agent
- 用户点击"自定义策略"
- `用户勾选允许的标签`：[B2B, READ_ONLY]
- `用户配置数据范围`：datasourceIds=[184]
- `用户配置A2A权限`：enabled=true, allowedTargetAgents=[agent-002]
- 用户保存委托
...
- Agent调用datasource_delete工具
- MCP Gateway返回403错误：USER_CONFIRMATION_REQUIRED
- Agent返回实时确认提示给Copilot
- 用户点击确认链接
- 权限中心弹出确认窗口
- 用户点击"确认执行"
- 权限中心创建临时委托
...
- MCP Gateway验证Token有效性
- Token状态为EXPIRED
- MCP Gateway返回401错误：TOKEN_EXPIRED
...
- Agent连续调用工具（频率异常）
- MCP Gateway监控调用频率
- MCP Gateway检测频率异常（>100次/分钟）
- MCP Gateway触发安全防护
- MCP Gateway立即失效Token
- MCP Gateway终止Task
- MCP Gateway返回403错误：SECURITY_VIOLATION
...
- Agent Gateway定时健康检查
- Agent返回错误或超时
- Agent Gateway更新Agent状态为UNHEALTHY
- 用户提交任务请求
- Agent Gateway拒绝生成Token
