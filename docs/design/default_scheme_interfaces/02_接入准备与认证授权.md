...
```json
{
  ...
  "callback_base_url": "https://agent.local/callback"
}
```
...
Agent-->>User: 302 到 next_action.redirect_url
...
**调用方**：接入 Agent  
**被调用方**：Agent 网关  
**用途**：获取当前用户请求对应的 `TR`；如果前置登录/授权尚未完成，则返回继续流程所需的 `request_id + redirect_url`
...
```json
{
  ...
  "data": {
    ...
    "redirect_url": "https://gateway.local/api/v1/auth/login/start?request_id=req_sec_001&agent_id=agt_business_001&client_return_url=https%3A%2F%2Fagent.local%2Fruntime%2Fsecurity%2Fresult&state=st_login_001"
  }
}
```
...
- 在 `LOGIN_REQUIRED` 阶段，Agent 网关不会提前调用策略中心
...
| `client_return_url` | 登录完成后回到接入 Agent 的地址 |
| `state` | 防 CSRF 状态值 |
...
- Agent 网关把当前 `request_id` 标记为“已登录”
- 写入网关侧用户身份上下文
- 302 回跳 `client_return_url?request_id=req_sec_001&status=LOGIN_COMPLETED`
...
- Agent 网关保存 `Tc`
- 302 回跳 `client_return_url?request_id=req_sec_001&status=CONSENT_COMPLETED`
- 这一阶段只覆盖到 `Tc` 获取
- `T1` 获取、`TR` 生成以及 `READY + session_id + tr_token` 的返回，统一放在 [03_T1与TR生成.md](/D:/IDEA_Project/init_env/auth-design-spec/docs/design/default_scheme_interfaces/03_T1与TR生成.md)
