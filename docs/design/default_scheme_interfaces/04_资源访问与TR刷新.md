...
Agent->>Gateway: POST /api/v1/runtime/sessions/{session_id}/tr/refresh
Gateway->>Gateway: 校验 session_id + Agent 运行时身份
Gateway->>Gateway: 读取 session.current_refresh_token_ref
...
Gateway-->>Agent: 200 { status=READY, session_id, tr_token }
```

### 2.2 `POST /api/v1/runtime/sessions/{session_id}/tr/refresh`
...
1. 校验 `session_id` 和 Agent 运行时身份
2. 读取 `session.current_refresh_token_ref`
3. 调用 IAM 真实接口：
...
```json
{
  ...
  "message": "session_id is invalid or expired",
  "data": {
    "session_id": "ses_not_found"
  }
}
```
...
> 说明：
>
> - `session_id` 只是会话索引，不是单独可换 `TR` 的凭证
> - 刷新接口仍然要求 Agent 运行时身份校验
> - `refresh_token` 保存在 Agent 网关侧 session 中，不返回给接入 Agent
...
- Agent 网关应成为 `Tc / T1` 的唯一持有者
- Agent 网关也应成为 `refresh_token` 的唯一持有者
- 接入 Agent 仅缓存 `session_id + 当前 TR`
- `TR` 刷新逻辑必须走 Agent 网关，不允许接入 Agent 直连 IAM
- 刷新主路径优先走 IAM 的 `/iam/auth/refresh-resource-token`

## 4. 本文档未覆盖内容

- 插件方案接口
- Agent 运行时身份认证的具体技术选型
- Token 签名算法、密钥管理与证书细节
- 生产级审计、限流、重试与灾备策略
