...
Gateway->>Gateway: 创建 session 并绑定当前 TR / refresh_token / effective_resources
...
写回 `security_request` 和后续 `session`，供 MCP 调用阶段使用。

## 5. `POST /api/v1/runtime/tr/acquire` 在 `READY` 时的响应

> 说明：这里的“同步返回”指 Agent 在浏览器回跳后再次调用同一个 `TR` 获取接口，拿到最终结果，而不是由 Agent 网关主动回调推送。
...
对接入 Agent 来说，`POST /api/v1/runtime/tr/acquire` 只会看到两类结果：

- `READY`
  - 直接拿到 `session_id + tr_token`
- `REDIRECT_REQUIRED`
  - 按 `redirect_url` 跳转，并在浏览器回跳后带着 `request_id` 再调一次同一个接口
...
- `session` 只在当前 `security_request` 首次进入 `READY` 时创建
- `session` 创建后，Agent 网关把：
  - `request_id`
  - `agent_id`
  - `user_id`
  - `tc_ref`
...
- 后续 `TR` 刷新会更新：
  ...
  - `current_refresh_token_ref`
  不再新建 `session`
- 因此，接入 Agent 持有的 `session_id` 可以稳定复用，而 `TR` 可以滚动更新
