...
| MCP 网关 | MCP 调用入口、code → tool 反查、路由分发 | 接受 `TR`；基于其中授权 code 反查允许访问的 `MCP tool`；把调用路由到对应 `MCP 服务` | 不处理登录授权跳转；不签发 `Tc / T1 / TR` |
...
**调用方**：业务 Agent 后端/BFF  
**被调用方**：`MCP 网关`  
**用途**：带 `TR` 访问 `MCP 网关`，由 `MCP 网关` 基于授权 code 反查允许访问的 `MCP tool`，再路由到对应 `MCP 服务`

#### 说明

- 业务 Agent 只和 `MCP 网关` 交互，不直接面向具体 `MCP 服务`
- `MCP 网关` 接受 `TR`
- `MCP 网关` 基于 `TR` 中的授权 code 识别允许调用的 `MCP tool`
