...
长期/永久授权不等于长期 `TR`。长期授权记录由 `IDaaS` 作为权威保存，表示“某用户已经长期同意某业务 Agent 使用某些权限点”；`TR` 仍然是短期资源访问令牌，只表达当前这次可用于访问资源的权限点上限边界。
...
- 如果用户在 IDaaS 侧选择长期授权，后续业务 Agent 仍按原流程申请新的短期 `TR`；区别只是 IDaaS 可在 authorize 阶段命中长期授权记录并静默/快速返回 `code`。
...
sequenceDiagram
    ...
    用户->>IDaaS: GET /oauth2/authorize
    IDaaS-->>用户: 首次授权展示授权页；命中长期授权可静默返回
    用户->>IDaaS: 勾选并确认授权
...
- 长期授权只影响 IDaaS 是否展示授权确认页，不改变 Agent 网关的一次性票据和短期 `TR` 模型。
...
sequenceDiagram
    ...
    alt 本地有有效 TR 且覆盖这组工具
        ...
        用户->>IDaaS: GET /oauth2/authorize
        IDaaS-->>用户: 首次授权展示授权页；命中长期授权可静默返回 code
        用户->>IDaaS: 勾选并确认授权
        IDaaS-->>用户: 302 回 /gw/auth/consent/callback?code&state
        用户->>GW: GET /gw/auth/consent/callback?code&state
        GW->>IDaaS: code 换 Tc
...
- 业务 Agent 换回 `TR` 后，必须校验 `TR.agency_user` 与当前 `site_session` 用户一致。
- 长期授权记录不进入 MCP 运行时鉴权；MCP 网关只信当前请求携带的短期 `TR`、策略中心和当前工具。
...
- `TR` 始终是短期资源访问令牌，长期授权记录由 IDaaS 保存，不是长期 `TR`。
- Agent 网关不维护长期登录态，不再把登录用户桥接成长期网关会话。
- Agent 网关不保存长期授权账本，不提供当前阶段的后端无感刷新。
- IDaaS 负责判断用户是否已经登录，必要时展示登录页或授权页。
- 如果 IDaaS 命中长期授权记录，可以在 authorize 阶段不展示授权页并快速回跳。
