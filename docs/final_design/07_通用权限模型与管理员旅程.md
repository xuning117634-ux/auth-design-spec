...
举一个完整例子：管理员想配置“用户 `Y30037812` 不允许访问合同助手”。

| 属性 | 示例值 | 含义 |
| --- | --- | --- |
| `Subject` | `USER:Y30037812` | 谁发起访问，这里是具体用户 `Y30037812`。 |
| `Resource` | `AGENT:contract_agent_001` | 要访问什么资源，这里是合同助手 Agent。 |
| `Action` | `agent.access` | 要做什么动作，这里是访问 Agent。 |
| `Context` | `enterprise=11111111111111111111111111111111` | 这次判断发生在哪个企业或场景下。 |
...
策略中心收到访问判断请求时，会把这四部分放在一起判断；如果命中这条 `DENY`，就返回 `DENY`，业务 Web 不展示或不允许进入合同助手。
