...
```text
Subject 主体 + Action 动作 + Resource 资源 + Context 场景 => ALLOW / DENY
```
...
`OPEN` 适合普通 Agent 或普通 Skill 的使用控制。
...
`RESTRICTED` 适合敏感 Agent 或敏感 Skill 的使用控制。
...
| 顶部 Agent 概览 | 展示 Agent 名称、`agentId`、访问模式、已启用 Skill、订阅权限点。这里的能力清单是只读信息，来源于 Agent 注册面或同步数据。 |
