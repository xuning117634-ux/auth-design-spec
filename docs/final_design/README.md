...
| 理解通用权限模型与 Agent 优先管理面 | [07_通用权限模型与管理员旅程.md](./07_通用权限模型与管理员旅程.md) |
| 理解未来 Cedar 接入预留 | [08_Cedar接入改造提醒.md](./08_Cedar接入改造提醒.md) |
...
| `07_通用权限模型与管理员旅程.md` | 策略中心通用权限模型演进、Agent 优先管理面，以及业务管理员从 MCP、Agent 到策略配置的操作旅程。 |
| `08_Cedar接入改造提醒.md` | 未来接入 Cedar 策略引擎时的 JSON IR、编译路径、执行器架构和风险提醒。 |
...
| 通用权限模型、PARC、Cedar 预留 | `07_通用权限模型与管理员旅程.md`、`08_Cedar接入改造提醒.md`、必要时同步 `CHANGELOG.md` |
